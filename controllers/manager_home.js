...
router.get("/fakeuser", (req, res) => {
  ...
  res.render("manager/fakeuser", data);
});
...
router.get("/user_list", (req, res) => {
  userModel.getAll(function (results) {
    ...
    res.render("manager/user_list", data);
  });
});

router.get("/delete/:id", function (req, res) {
  userModel.delete(req.params.id, function (status) {
    res.redirect("/home-manager/user_list");
  });
});
...
router.post("/user_edit/:id", function (req, res) {
  ...
  userModel.IndividualUserUpdate(user, function (status) {
    if (status) {
      ...
      res.redirect("/home-manager/user_edit/" + req.params.id);
    }
  });
...
//edit admin profile
router.get("/manager-edit_account", (req, res) => {
  userModel.get(req.session.user_id, result_info => {
    if (result_info.id != null) {
      ...
      res.render("manager/edit_profile", data);
    } else {
...
router.post("/manager-edit_account", (req, res) => {
    ...
    bcrypt.hash(update_user.user_password, saltRounds, function (err, hash) {
      ...
      userModel.update(update_user, user_update_status => {
        if (user_update_status) {
          ...
          res.redirect("/home-manager/manager-edit_account");
        }
      });
    });
  } else {
...
router.get("/add_users", (req, res) => {
  ...
  res.render("manager/add_users", err);
});
  ...
  if (!err) {
    ...
    bcrypt.hash(user.user_password, saltRounds, function (err, hash) {
      ...
      userModel.insert(user, function (results) {
        if (results) {
          req.session.success = "User added!!";
          res.redirect("/home-manager/add_users");
        } else {
          req.session.success = "Probelm with add new user";
          res.redirect("/home-manager/user_list");
        }
...
router.get("/add_restaurants", (req, res) => {
  ...
  res.render("manager/add_restaurants", data);
});
        ...
        userModel.getRestaurant(data, function (result) {
          if (result) {
            ...
            userModel.insertIntoRestaurantItem(data, function (st) {
              if (st) {
                res.redirect("/home-manager/restaurants_list");
              }
            });
          }
        });
      } else {
        res.redirect("/home-manager/add_restaurants");
      }
    });
  } else {
    req.session.errors = err;
...
router.get("/restaurants_list", (req, res) => {
  userModel.getAllRestaurant(function (result) {
    if (result.length) {
      ...
      res.render("manager/restaurants_list", data);
    }
  });
});
    ...
    if (r_results.length != 0) {
      userModel.getAllItems(req.params.id, i_results => {
        if (i_results.length != 0) {
          ...
          res.render("manager/edit_restaurant", data);
        }
      });
    } else {
      res.redirect("/home-manager/restaurants_list");
...
router.post("/edit-item/:id", (req, res) => {
  ...
  userModel.updateItem(data, status => {
    res.redirect("/home-manager/edit/" + req.session.r_id);
  });
});
...
router.get("/view-restaurant", (req, res) => {
  userModel.getRestaurantByLocation(req.session.u_loc, results => {
    ...
    res.render("manager/view_restaurant_list", data);
  });
});
...
router.get("/contact_list", (req, res) => {
  userModel.getAllFromContact(req.session.user_id, result => {
    ...
    res.render("manager/contact_list", data);
  });
});
...
//   });
  ...
  router.post("/reservation_edit/:id", function (req, res) {
    ...
    userModel.ReservationUpdate(data, function (status) {
      if (status) {
        res.redirect("/home-manager/all_reservations");
      } else {
        res.redirect("/home-manager/reservation_edit/" + req.params.id);
      }
    });
  });
  ...
  router.get('/food_experience', (req, res) => {
    userModel.getAllFoodExperience((results) => {
        ...
        res.render('manager/food_experience', data);
    });
});

router.post('/food_experience', (req, res) => {
    var data = {
        u_id: req.session.user_id,
        u_name: req.session.user_name,
    ...
    userModel.insertFoodExperience(data, (staus) => {
        res.redirect('/home-manager/food_experience');
    });
});
